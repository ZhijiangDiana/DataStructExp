...
public:
    ...
    void print(){  //打印链表数据
        ...
        thisElem=first->next;
        while(thisElem!=NULL){
            std::cout<<thisElem->data<<std::endl;
            thisElem=thisElem->next;
        }
    }
    bool add(int position,const T &insertData){  //后插操作
    ...
    bool add(const T &insertData){  //尾插操作
        DNode<T> *thisElem=first;
    ...
    bool replace(int position,const T &insertData){  //给定位置修改值
        if(position>length||length==0||position<=0){
            return false;
        }
